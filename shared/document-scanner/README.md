...
`new DocumentScanner({ maxLongSide: 2400 }).process(file, labels)` возвращает после подтверждения `{ confirmed, blob, width, height, filter, corners, documentDetected, usedOriginal, quality }`.
