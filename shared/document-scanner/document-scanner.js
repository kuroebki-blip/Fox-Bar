(function(root) {
  ...
  function detect(canvas, cv) { let src, small, gray, blur, edges, softEdges, binary; try { src = cv.imread(canvas); const scale = Math.min(1, 720 / Math.max(src.cols, src.rows)); small = new cv.Mat(); cv.resize(src, small, new cv.Size(Math.round(src.cols * scale), Math.round(src.rows * scale))); gray = new cv.Mat(); blur = new cv.Mat(); edges = new cv.Mat(); softEdges = new cv.Mat(); binary = new cv.Mat(); cv.cvtColor(small, gray, cv.COLOR_RGBA2GRAY); cv.GaussianBlur(gray, blur, new cv.Size(5,5), 0); cv.Canny(blur, edges, 60, 160); cv.Canny(blur, softEdges, 28, 96); cv.threshold(blur, binary, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU); const frame = small.cols * small.rows; let best = null, bestArea = 0; [edges,softEdges,binary].forEach(source => { const contours = new cv.MatVector(), hierarchy = new cv.Mat(); try { cv.findContours(source, contours, hierarchy, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE); for (let i=0;i<contours.size();i+=1) { const contour=contours.get(i), approx=new cv.Mat(); try { cv.approxPolyDP(contour, approx, .018*cv.arcLength(contour,true), true); const area=cv.contourArea(approx); if (approx.rows===4 && cv.isContourConvex(approx) && area>frame*.08 && area<frame*.985 && area>bestArea) { const points=[]; for(let j=0;j<4;j+=1){const p=approx.intPtr(j,0);points.push({x:p[0]/scale,y:p[1]/scale});} const corners=G.sortCorners(points); if(G.isConvex(corners)){best=corners;bestArea=area;} } } finally { approx.delete(); contour.delete(); } } } finally { contours.delete(); hierarchy.delete(); } }); return best ? { corners:best, areaRatio:bestArea/frame } : null; } finally { [src,small,gray,blur,edges,softEdges,binary].forEach(mat=>{if(mat)mat.delete();}); } }
