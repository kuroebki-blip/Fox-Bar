...
const start = frontend.indexOf('function buildYandexMapsExternalRouteUrl');
const end = frontend.indexOf('async function loadRideRouteDetails', start);
const buildYandexMapsExternalRouteUrl = new Function(`${frontend.slice(start, end)}; return buildYandexMapsExternalRouteUrl;`)();
...
test('external Yandex Maps route URL preserves the supplied 1, 2, or 3 dropoff points', () => {
  for (const count of [1, 2, 3]) {
    const url = new URL(buildYandexMapsExternalRouteUrl(origin, points.slice(0, count)));
...
test('Yandex route controls use optimizer dropoffOrder, Telegram external navigation and rides.optimize', () => {
  ...
  assert.match(frontend, /TG\.openLink\(buildYandexMapsExternalRouteUrl/);
  assert.doesNotMatch(frontend, /yandexmaps:\/\//);
