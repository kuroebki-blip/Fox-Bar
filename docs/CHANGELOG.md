...
- основной и повторный OCR терминальных слипов отдельно сравнивают форму цифр `0`, `6` и `8` во всех перекрывающихся секторах;
- запрещено превращать `0` в `8` из-за шума, блика или сгиба термобумаги;
- арифметическая проверка компонентов сохранена, подгонка результата под iiko запрещена;
- неоднозначный слип направляется на ручную проверку вместо угадывания суммы.

Как откатиться:

- Apps Script deployment переключить на version `24` (`v9.6.0 TERMINAL DETAIL MONTAGE`).
...
- для кассового отчёта Tatooine удалена проверка Telegram ID по списку разрешённых пользователей;
- приложением может пользоваться любой пользователь с валидной Telegram Mini App-авторизацией;
- отдельный бот, целевой чат и ограничение Tatooine только кассовыми действиями сохранены;
- FO’X и остальные маршруты доступа не изменены;
- ограничение пользователей Tatooine не возвращать без прямого указания владельца.

Как откатиться:

- переключить Apps Script deployment обратно на version `21` (`v9.4.9`).
