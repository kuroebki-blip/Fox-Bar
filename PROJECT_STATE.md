...
- Снятие резерва после завершения и добавление новых позиций требуют проверки на фактическом production backend.
...
Candidate: ESLint в режиме проверки готовится в отдельной ветке `tooling/eslint-check`. На этом этапе анализируются только самостоятельные browser scripts Tatooine; рабочий код не форматируется и не исправляется.

## Следующие шаги

1. Изучить диагностический отчёт ESLint и отдельно согласовать, какие найденные проблемы исправлять.
