...
- Терминальные чеки ранее могли распознавать цифры `0`, `6` и `8` неверно; пользователь сообщил об ухудшении на снимках встроенной Android-камеры Tatooine `v1.5.0`.
...
Candidate: Tatooine `v1.5.1` запрашивает полноразмерное фото через Android `ImageCapture`; стоп-кадр preview остаётся fallback. iPhone сохраняет существующий file-input путь. Живой тест точности на Android ещё не выполнен.

## Следующие шаги

1. Проверить разрешение и точность полноразмерного снимка `v1.5.1` внутри Telegram на Android.
