...
- Tatooine `v1.10.7`: добавлен отдельный release path с самостоятельной HTML-страницей для запуска из Telegram. Он исключает повторное использование Mini App HTML, который Telegram кеширует по прежнему пути; функциональность приложения и backend не изменяются.
